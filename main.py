...
player2 = ai.WordleAI()
hist = []
print("Welcome to wordle solver. When you enter your word, each letter will be either E,Y, or G.\n E is Empty, Y is Yellow, and G is Green.")
must_have = set()
not_have = set()
letters = {'a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z'}
pPlay = True
while tries <= 6:
    print("possible letters:", sorted(list(letters-not_have)))
    tries += 1
    if pPlay: 
        user_word = input(f"Enter your word: ").lower()
        s = []
        ...
        freq_guess = dict()
        for i in range(5):
            if user_word[i] in freq_guess:
                freq_guess[user_word[i]] += 1
            else:
                freq_guess[user_word[i]] = 1
            
            if user_word[i] == ans[i]:
                must_have.add(user_word[i])
                if user_word[i] in not_have:
                    not_have.remove(user_word[i])
                s.append("G")
                if freq_guess[user_word[i]] > freq_ans[user_word[i]]:
                    for j in range(len(s)):
                        if s[j] == "Y" and user_word[j] == user_word[i]:
                            s[j] = "E"
                            break
            elif user_word[i] in ans and freq_guess[user_word[i]] > freq_ans[user_word[i]]:
                s.append("E")
            elif user_word[i] in ans:
                s.append("Y")
                must_have.add(user_word[i])
                if user_word[i] in not_have:
                    not_have.remove(user_word[i])
                ...
                s.append("E")
                not_have.add(user_word[i])
        print(" "*17,end="")
        print(''.join(s))
    user_word = player2.getBestWord()
    hist.append(user_word)
    s = []
    ...
    if freq["g"] == 5:
        if ptries > tries: 
            ...
            print("You won!")
        print("Your opponent guessed:\n"+"\n".join(hist))
        print("The word was:",ans)
        exit()
    print(f"Your opponent got: {freq['e']} E's, {freq['y']} Y's, and {freq['g']} G's")
